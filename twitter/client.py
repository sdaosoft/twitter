...
class Client(BaseHTTPClient):
    ...
    async def request_(
        ...
        try:
            ...
            await self.relogin()
            return await self.request_(method, url, auto_relogin=False, **kwargs)
    ...
    async def _request_oauth2_auth_code(
        ...
        response, response_json = await self.request_("GET", url, params=querystring)
    ...
    async def _oauth(self, oauth_token: str, **oauth_params) -> requests.Response:
        ...
        response, _ = await self.request_("GET", url, params=oauth_params)
    ...
    async def _request_users_by_ids(
        ...
        for user_data in data["data"]["users"]:
            ...
            if user.twitter_id == self.account.twitter_id:
                users[self.account.twitter_id] = self.account
        return users
    ...
    async def upload_image(
        ...
        for attempt in range(attempts):
            try:
                response, data = await self.request_(
    ...
    async def view(self, tweet_id: int):
        event_data = [
            {
                ...
                "format_version": 2,
                "triggered_on": str(int(datetime.now().timestamp() * 1000)),
                "items": [
                    {
                        "item_type": 0,
                        "id": str(tweet_id),
                ...
                "client_event_sequence_start_timestamp": int(
                    datetime.now().timestamp() * 1000
                ),
                "client_event_sequence_number": 6,
        ...
        await self.request_("POST", url, data=data, headers=headers)
    ...
    async def _follow_action(self, action: str, user_id: int | str) -> bool:
        ...
        response, response_json = await self.request_(
    ...
    # async def view(self, tweet_id: int) -> dict:
    #     url, query_id = self._action_to_url('ViewTweet')
    #     variables = {
    ...
    #         'fieldToggles': field_toggles,
    #     }
    #     response, data = await self.request("GET", url, json=query_params)
    #     print(response, data)
    ...
    async def vote(
        ...
        response, response_json = await self.request_("POST", url, params=params)
    ...
    async def request_followers(
        ...
        if user_id:
            ...
            if not self.account.twitter_id:
                await self.update_account_info()
            return await self._request_users_by_action(
                "Followers", self.account.twitter_id, count, cursor
            )
    ...
    async def request_followings(
        ...
        if user_id:
            ...
            if not self.account.twitter_id:
                await self.update_account_info()
            return await self._request_users_by_action(
                "Following", self.account.twitter_id, count, cursor
            )
    ...
    async def request_tweets(
        ...
        if not user_id:
            if not self.account.twitter_id:
                await self.update_account_info()
            user_id = self.account.twitter_id
    ...
    async def _update_profile_image(
        ...
        response, data = await self.request_("POST", url, params=params)
    ...
    async def change_username(self, username: str) -> bool:
        ...
        response, data = await self.request_("POST", url, data=payload)
    ...
    async def send_message(self, user_id: int | str, text: str) -> dict:
        ...
        response, data = await self.request_("POST", url, json=payload)
    ...
    async def unlock(self):
        ...
        response, html = await self.request_("GET", self._CAPTCHA_URL, bearer=False)
        (
            authenticity_token,
            assignment_token,
            needs_unlock,
    ...
    async def update_backup_code(self):
        ...
        response, response_json = await self.request_("GET", url)
    ...
    async def _send_raw_subtask(self, **request_kwargs) -> tuple[str, list[Subtask]]:
        ...
        response, data = await self.request_("POST", url, **request_kwargs)
        subtasks = [
        ...
        for subtask in subtasks:
            if subtask.id == "LoginAcid":
                ...
                if subtask.primary_text == "Check your email":
                    flow_token, subtasks = await self._login_email_auth_challenge(
                        flow_token
                    )
                try:
                    # fmt: off
    ...
    async def totp_is_enabled(self):
        ...
        url = f"https://x.com/i/api/1.1/strato/column/User/{self.account.twitter_id}/account-security/twoFactorAuthSettings2"
        response, data = await self.request_("GET", url)
        # fmt: off
